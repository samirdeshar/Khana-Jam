...
@push('styles')
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css" integrity="sha512-m7jw3qMi9BJ7Rm+O12Duvyt2ziOqU0CO0WP9YYtkP6rd4gO6E9HZaWZlrFNYuFSIgyBzZsFsv50I9C9lYjJgUw==" crossorigin="anonymous" />
       ...
       <div class="row">
          <div class="col-xl-8">
             <div class="recent-news-two">
                ...
                <div class="recent-news mt-3">
                   <div>
                      ...
                      <h2>{{$data->name}}</h2>
                      <div class="d-flex align-items-center"><img alt="img" class="me-3" src="assets/img/man.jpg"><h6>by Thomas Walimes</h6>
                      </div>
                   </div>
                </div>
             </div>
             ...
             <div class="comment">
                ...
                <form method="POST" action="{{ route('save-comment') }}" class="add-review leave-comment comment">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 ps-lg-0">
                            {!! Form::text('name', '', ['class' => 'form-control ' . ($errors->has('name') ? 'is-invalid' : ''), 'placeholder' => 'Full Name .....', 'required' => true]) !!}
                            @error('name')
                        ...
                        {!! Form::textarea('comment', '', ['class' => 'form-control ' . ($errors->has('comment') ? 'is-invalid' : ''), 'placeholder' => 'Comments .....', 'required' => true, 'rows' => 5, 'style' => 'resize:none;']) !!}
                        @error('comment')
                            <span class="text-danger">{{ $message }}</span>
                        @enderror
                ...
                @else
                <p>Please <a href="{{ url('/customer/signup'    ) }}">Login</a> to post a comment and provide a rating.</p>
                @endauth
            ...
            <div class="posts recent-posts">
                ...
                <ul>
                    @foreach ($recommendations as $recommendation)
                        <li>
                            <img alt="img" src="{{ asset($recommendation->image) }}" style="height: 100px; width:100px;">
                            <div>
                                <a href="{{ route('res_details', ['slug' => $recommendation->slug]) }}">{{ $recommendation->updated_at->format('F j, Y') }}</a>
